...
public class Parser {
	...
	public Parser(String text) {
		...
		while (dd != null) { // 다음 블럭이 없거나 에러가 날때까지 읽습니다.
			...
			if (needBody != null) {
				...
				needBody.setBody(obj.getFields(), dd[1]); // 사용되는 필드가 자동으로 검색됩니다.
			}
			// 다음 블럭을 받아옵니다.
			dd = nextBlock();
		}
		...
		try {
			...
			while (token.hasMoreTokens()) {
				...
				/***************	생성자인지 확인합니다.	************/	
					...
					/***************	접근범위 설정인지 확인끝		************/
					if (accessModified) // 만약 이 줄로 접근범위가 설정되있었다면 다음줄로 건너 뜁니다.
						continue;
	...
	private Method findMethod(String part) {
		//선언정보를 가지고 있는 문자 에서 파라메터정보가 시작 되는 부분의 위치를 찾습니다.
